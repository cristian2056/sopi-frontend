...
export default function TabAsignacion({ equipoId, crear, modificar, eliminar }) {
  ...
  const [dependenciasMap, setDependenciasMap] = useState({});

  const cargar = async () => {
    setLoading(true); setError("");
    try {
      const [data, rU, rD] = await Promise.all([
      ...
      const uMap = {};
      (rU.datos ?? []).forEach(u => { uMap[u.usuarioId] = u.nombreCompleto ?? u.userName ?? `#${u.usuarioId}`; });
      setUsuariosMap(uMap);

      const dMap = {};
      (rD.datos ?? []).forEach(d => { const id = d.dependeciaId ?? d.dependenciaId; dMap[id] = d.nombre; });
      setDependenciasMap(dMap);
    } catch (e) {
            ...
            <div key={asig.equipoAsignacionId} style={{ background: "#fff", border: "1.5px solid #e5e7eb", borderRadius: 10, padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: "0.97rem", color: "#232946", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  {asig.usuarioNombre ?? usuariosMap[asig.usuarioResponsableId] ?? `Usuario #${asig.usuarioResponsableId}`}
                  <TipoUsuarioBadge tipo={asig.tipoUsuario} />
                  {(asig.dependenciaNombre ?? dependenciasMap[asig.dependeciaId]) && (
                    <span style={{ color: "#6b7280", fontWeight: 400 }}>· {asig.dependenciaNombre ?? dependenciasMap[asig.dependeciaId]}</span>
                  )}
                </div>
