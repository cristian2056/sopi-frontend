...
export default function SeccionUsuario({ equipos, tickets, loadingEquipos, loadingTickets, onCrearTicket }) {
  ...
  const cerradosVisibles = verHistorial ? cerrados : cerrados.slice(0, 2);
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
            <h3 style={{ margin: 0, fontWeight: 800, fontSize: "1rem", color: "#111827" }}>
              ✅ Resueltos
            </h3>
            {cerrados.length > 2 && (
              <button onClick={() => setVerHistorial(v => !v)} style={{
                background: "none", border: "none", cursor: "pointer",
                color: "#4c7318", fontWeight: 600, fontSize: "0.82rem", padding: 0,
              }}>
                {verHistorial ? "Ocultar historial" : `Ver historial (${cerrados.length})`}
              </button>
            )}
          </div>
          <ListaTickets tickets={cerradosVisibles} loading={loadingTickets} vacio="No tienes tickets resueltos." />
        </div>
      </div>
